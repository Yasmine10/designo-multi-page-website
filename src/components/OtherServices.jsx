...
function OtherServices(props) {
    console.log(props.services[0].image);

    return (
        <section className="service-other-services service | container">
            { props.services.map((service, index) => (

                <Link to={`/services/${service.slug}`} className="service-other-services__service-link" key={index}>
                        <div className="bg">
                            <picture>
                                { service.slug === 'web-design' ?
                                    <>
                                        <source srcSet={`/assets/home/desktop/${service.images.small}`} media="(min-width: 1440px)" />
